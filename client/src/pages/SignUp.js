...
const SignUp = ({ setIsLoggedIn }) => {
  ...
  const sendOtp = async () => {
    try {
      ...
      const response = await axios.post(`${process.env.VITE_REACT_APP_BACKEND_BASEURL}/api/auth/send-otp`, {
        phone: data.phone,
        email: data.email
      });
  ...
  const verifyOtp = async () => {
    try {
      const data = isUser ? userData : roomOwnerData;
      const response = await axios.post(`${process.env.VITE_REACT_APP_BACKEND_BASEURL}/api/auth/verify-otp`, {
        phone: data.phone,
        email: data.email,
        otp
      });
  ...
  const handleSubmit = async (e, isUserForm) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.VITE_REACT_APP_BACKEND_BASEURL}/api/auth/register${isUserForm ? '' : '-roomowner'}`,
        formData,
        {
